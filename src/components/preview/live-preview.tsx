...
function generateMultiFileHtmlDocument(
  ...
  // Generate viewport meta tag — like Chrome DevTools device mode.
  // The page always renders at initial-scale=1.0.  Visual scaling is handled
  // externally via CSS transform on the iframe wrapper (never inside the page).
  const getViewportMeta = () => {
    if (isResponsive || !viewportWidth) {
      // Responsive mode or no explicit width — behave like a normal browser
      return `<meta name="viewport" content="width=device-width, initial-scale=1.0">`;
    }
    // Device mode — tell the page its CSS viewport is exactly this device's width.
    // initial-scale=1 so media-queries & CSS see the real device width.
    return `<meta name="viewport" content="width=${viewportWidth}, initial-scale=1.0">`;
  };
...
export function LivePreview({ files, framework, className, onRun }: LivePreviewProps) {
  ...
  const [zoom, setZoom] = useState<string>("1"); // "0.5" | "0.75" | "1" | "1.25" | etc. — relative to fit
  ...
  // ── Auto-fit base scale ─────────────────────────────────────────────
  // Calculates the scale factor needed to fit the device frame inside the
  // available container area.  CAN upscale so the device always fills the
  // panel — zoom percentages are relative to this baseline.
  const fitScale = useMemo(() => {
    if (selectedDevice === "Responsive") return 1;

    const dw = dimensions.width;
    const dh = dimensions.height;
    if (typeof dw !== "number" || typeof dh !== "number" || dw <= 0 || dh <= 0) return 1;

    const cw = containerDimensions.width;
    const ch = containerDimensions.height;
    if (cw <= 0 || ch <= 0) return 1;

    // Scale to fill the panel (no 1:1 cap — small devices scale UP)
    return Math.min(cw / dw, ch / dh);
  }, [selectedDevice, dimensions, containerDimensions]);

  // ── Effective visual scale ────────────────────────────────────────────
  // 100 % = fit-to-panel.  Other zoom levels are relative to fit:
  //   50 % → fitScale × 0.5,  200 % → fitScale × 2, etc.
  // In responsive mode fitScale is 1, so zoom is 1:1 with CSS pixels.
  const effectiveScale = useMemo(() => {
    const zoomFactor = parseFloat(zoom) || 1;
    return selectedDevice === "Responsive" ? zoomFactor : fitScale * zoomFactor;
  }, [zoom, selectedDevice, fitScale]);

  // ── Generate preview HTML ─────────────────────────────────────────────
  // Zoom is NOT passed into the HTML document.  The page always renders at
  // native device resolution; visual scaling is CSS-transform only.
  const srcDoc = useMemo(
    () => {
      ...
      return generateMultiFileHtmlDocument(files, framework, width, height, selectedDevice === "Responsive");
    },
    [files, framework, dimensions.width, dimensions.height, selectedDevice]
  );
  ...
  const handleDeviceChange = (value: string) => {
    ...
    setZoom("1"); // Reset to 100 % (fit-to-panel) when switching devices
  ...
  return (
    ...
    >
      ...
      {/* Responsive Toolbar — single scrollable row that never wraps */}
      <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border/40 bg-background/30 backdrop-blur-sm overflow-x-auto scrollbar-none min-w-0">
        {/* Device selector */}
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[11px] font-medium text-muted-foreground whitespace-nowrap hidden sm:inline">Dimensions:</span>
          <Select value={selectedDevice} onValueChange={handleDeviceChange}>
            <SelectTrigger className="h-7 w-[110px] sm:w-[140px] text-xs py-0 px-2 bg-background/50 border-border/40 hover:bg-background/80 transition-colors">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {DEVICES.map((device) => (
        ...
        <div className="flex items-center gap-1 shrink-0">
          <Input
            ...
            className="h-7 w-12 sm:w-14 text-xs px-1 text-center bg-background/50 border-border/40 focus-visible:ring-primary/20"
          />
        ...
        {/* Rotate — only for fixed-size devices */}
        ...
        {/* Zoom selector — 100 % = fills the panel; other levels are relative */}
        <Select value={zoom} onValueChange={setZoom}>
          <SelectTrigger className="h-7 w-[76px] sm:w-[84px] text-xs py-0 px-2 bg-background/50 border-border/40 shrink-0" title={`Visual scale: ${Math.round(effectiveScale * 100)}%`}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {ZOOM_LEVELS.map((z) => (
        ...
        {/* Utility actions — pushed to far right */}
      ...
      {/* ── Preview area ─────────────────────────────────────────────────
           ...
           • Responsive mode  → iframe fills container 1:1, resize handles
           • Device mode      → iframe at exact device CSS-pixel size,
                                 visually scaled via CSS transform to fit panel
           Visual zoom is *only* a CSS transform — the page inside always
           renders at the device's native resolution.
      ──────────────────────────────────────────────────────────────────── */}
      <div
        ref={containerRef}
        className={cn(
          "relative flex-1 bg-muted/20 overflow-auto flex items-start justify-center p-4",
      ...
      >
        ...
        {/* Device frame wrapper — scaled visually, never affects layout inside */}
        <div
          style={{
            width: `${dimensions.width}px`,
            height: `${dimensions.height}px`,
            // Pure visual scaling — the page inside still renders at native
            // device resolution (e.g., 430 CSS-px for iPhone 14 Pro).
            transform: effectiveScale !== 1 ? `scale(${effectiveScale})` : undefined,
            transformOrigin: "top center",
            transition: "transform 0.15s ease-out",
            boxShadow: selectedDevice !== "Responsive"
              ? "0 8px 32px -8px rgba(0,0,0,0.45)"
              : "none",
          }}
          className={cn(
            "relative bg-background overflow-hidden shrink-0",
            selectedDevice !== "Responsive" && "border border-border/50 rounded-xl"
          )}
        >
          {/* Resize handles — responsive mode only */}
          {selectedDevice === "Responsive" && (
            <>
              {/* Right handle */}
              <motion.div
                ...
                onDragStart={() => {
                  if ((!customWidth || customWidth === "") && iframeRef.current) {
                    setCustomWidth(Math.round(iframeRef.current.offsetWidth / effectiveScale).toString());
                  }
                }}
                onDrag={(_, info) => {
                  const current = parseInt(customWidth || "0") || containerDimensions.width;
                  setCustomWidth(Math.max(200, current + Math.round(info.delta.x / effectiveScale)).toString());
                }}
              ...
              <motion.div
                ...
                onDragStart={() => {
                  if ((!customHeight || customHeight === "") && iframeRef.current) {
                    setCustomHeight(Math.round(iframeRef.current.offsetHeight / effectiveScale).toString());
                  }
                }}
                onDrag={(_, info) => {
                  const current = parseInt(customHeight || "0") || containerDimensions.height;
                  setCustomHeight(Math.max(200, current + Math.round(info.delta.y / effectiveScale)).toString());
                }}
              ...
              <motion.div
                ...
                onDrag={(_, info) => {
                  const curW = parseInt(customWidth || "0") || containerDimensions.width;
                  const curH = parseInt(customHeight || "0") || containerDimensions.height;
                  setCustomWidth(Math.max(200, curW + Math.round(info.delta.x / effectiveScale)).toString());
                  setCustomHeight(Math.max(200, curH + Math.round(info.delta.y / effectiveScale)).toString());
                }}
          ...
          {/* The iframe renders at the device's native CSS-pixel size.
              Visual fitting is handled by the wrapper's CSS transform. */}
          <iframe
            key={key}
            ref={iframeRef}
            src="/preview/frame"
            ...
            credentialless="true"
            onLoad={handleLoad}
            className="border-0 relative z-10 block"
            style={{
              width: `${dimensions.width}px`,
              height: `${dimensions.height}px`,
            }}
              ...
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    transform: `scale(${effectiveScale * 4})`,
                    transformOrigin: `${mouseX.get() - ((containerRef.current?.clientWidth || 0) - (iframeRef.current?.clientWidth || 0) * effectiveScale) / 2}px ${mouseY.get() - 16}px`,
                    position: 'absolute',
                    left: 0,
                    top: 0
                  }}
