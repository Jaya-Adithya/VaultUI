...
export function Playground({ componentId }: PlaygroundProps) {
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* Header */}
      <header className="flex flex-wrap items-center gap-x-3 gap-y-1.5 px-3 py-2 border-b border-border bg-background shrink-0 min-w-0 overflow-x-auto sm:px-4 sm:gap-x-4 sm:flex-nowrap">
        {/* Left group: back + isolation warning + title + badges */}
        <div className="flex items-center gap-2 min-w-0 flex-1 sm:gap-3">
          ...
          {/* Isolation Warning — icon-only on small screens */}
          {isIsolationMissing && (
            <div 
              className="flex items-center gap-1.5 text-amber-500 bg-amber-500/10 px-2 py-1 rounded text-xs border border-amber-500/20 shrink-0 cursor-help"
              title={isolationStatus.error || "WebContainer features require cross-origin isolation. Check console for details."}
            >
              <AlertTriangle className="h-3 w-3 shrink-0" />
              <span className="font-medium hidden sm:inline">Isolation Missing</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {isEditingTitle ? (
              <Input
                ...
                className="h-8 w-[160px] sm:w-[200px] text-sm"
                autoFocus
              />
            ) : (
              <button
                onClick={() => setIsEditingTitle(true)}
                className="flex items-center gap-1 text-base sm:text-lg font-semibold hover:text-primary transition-colors truncate min-w-0"
              >
            ...
            {/* Badges — hide file-count on very small screens */}
        ...
        {/* Right group: version history + actions */}
          ...
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {/* Version history accessible from menu on small screens */}
