...
function SignInContent() {
  ...
  useEffect(() => {
    ...
    // Check for OAuth/auth errors (e.g. redirected from /auth/error page or NextAuth)
    const authError = searchParams.get("error");
    if (authError) {
      console.log("[SignIn] Auth error from URL:", authError);
      let errorMessage = "An authentication error occurred.";
      
      switch (authError) {
        case "OAuthSignin":
          errorMessage = "Failed to start Google sign-in. The Google OAuth provider may not be configured on the server.";
          break;
        case "OAuthCallback":
          errorMessage = "Google sign-in callback failed. The redirect URI may not be registered in Google Cloud Console.";
          break;
        case "OAuthAccountNotLinked":
          errorMessage = "This Google account is not linked to your account. Please sign in with your email and password first.";
          break;
        case "OAuthCreateAccount":
          errorMessage = "Could not create account with Google. Please contact support.";
          break;
        case "AccessDenied":
          errorMessage = "Access denied. Your email must be from @position2.com domain.";
          break;
        case "CredentialsSignin":
          errorMessage = "Invalid email or password. Please try again.";
          break;
        case "Configuration":
          errorMessage = "Server configuration error. Google OAuth may not be properly configured.";
          break;
        case "Callback":
      ...
      // Clear the error parameter from URL (but keep displaying the error)
      router.replace("/auth/signin", { scroll: false });
    }
  ...
  const handleEmailPasswordSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Success! Keep loading state true while we redirect.
        // The component will unmount when navigation completes.
        window.location.href = callbackUrl;
      } else {
        // Handle case where result is neither ok nor has error
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      console.log("[SignIn] Starting Google OAuth flow with redirect: true...");
      
      // Use redirect: true for OAuth providers — this is the standard, most reliable approach.
      // The entire flow (redirect to Google → callback → session creation → redirect to app)
      // happens via page navigations, ensuring cookies are properly set at each step.
      // If any error occurs, NextAuth redirects to /auth/error which passes the error
      // back to this page via the "Back to Sign In" link.
      await signIn("google", {
        callbackUrl,
        redirect: true,
      });
      // This code won't execute because the page will navigate away
    } catch (err) {
      console.error("[SignIn] Google sign-in exception:", err);
