...
**Best Practices**:
- Set headers at the middleware level (not in individual routes)
...
3. Check middleware is running: Verify `src/middleware.ts` is being executed
...
- [ ] Middleware sets COOP/COEP headers for `/component/` and `/tools/` routes
